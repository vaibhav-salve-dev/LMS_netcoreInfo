    ...
    <div className="max-w-[1400px] mx-auto space-y-6">
      <div>
        ...
        <p className="text-sm text-ink-500 mt-1">
          Join your interactive sessions with instructors
        </p>
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-4 lg:gap-6">
        {upcomingClasses.map((c) => (
          <div key={c.id} className="card p-5 hover:shadow-card transition">
            <div
              className={`w-full h-32 rounded-xl bg-gradient-to-br ${colorBg[c.color]} flex items-center justify-center text-white mb-4`}
            >
              <Video size={36} />
            </div>
            ...
            <div className="flex items-center gap-4 mt-3 text-xs text-ink-500">
              ...
              <span className="flex items-center gap-1">
                <Users size={12} /> {c.duration}
              </span>
            </div>
            <button
              onClick={() => showToast(`Joining "${c.title}"...`)}
              className="mt-4 w-full py-2.5 rounded-xl bg-indigo-600 text-white text-sm font-semibold hover:bg-indigo-700 transition"
            >
              Join Class
            </button>
          </div>
